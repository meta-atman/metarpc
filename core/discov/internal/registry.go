...
func (c *cluster) handleWatchEvents(ctx context.Context, key watchKey, events []*clientv3.Event) {
	...
	for _, ev := range events {
		...
		default:
			logger.GetLoggerCtx(ctx).Errorf("Unknown event type: %v", ev.Type)
		}
	}
}
...
func (c *cluster) load(cli EtcdClient, key watchKey) int64 {
	...
	for {
		...
		logger.GetLoggerCtx(cli.Ctx()).Errorf("%s, key: %s, exactMatch: %t", err.Error(), key.key, key.exactMatch)
...
func (c *cluster) reload(cli EtcdClient) {
	...
	c.done = make(chan cast.PlaceholderType)
...
func (c *cluster) watch(cli EtcdClient, key watchKey, rev int64) {
	for {
		...
		if rev != 0 && errors.Is(err, rpctypes.ErrCompacted) {
			logger.GetLoggerCtx(cli.Ctx()).Errorf("etcd watch stream has been compacted, try to reload, rev %d", rev)
			rev = c.load(cli, key)
		}

		// log the error and retry
		logger.GetLoggerCtx(cli.Ctx()).Error(err)
	}
}
