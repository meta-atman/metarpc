...
type (
	...
	Publisher struct {
		...
		quit       *syncx.DoneChan
		pauseChan  chan cast.PlaceholderType
		resumeChan chan cast.PlaceholderType
	}
)
...
func NewPublisher(endpoints []string, key, value string, opts ...PubOption) *Publisher {
	publisher := &Publisher{
		...
		pauseChan:  make(chan cast.PlaceholderType),
		resumeChan: make(chan cast.PlaceholderType),
	}
...
func (p *Publisher) doKeepAlive() error {
	...
	for range ticker.C {
		...
		default:
			cli, err := p.doRegister()
			if err != nil {
				logger.GetLoggerCtx(cli.Ctx()).Errorf("etcd publisher doRegister: %s", err.Error())
				break
			}

			if err := p.keepAliveAsync(cli); err != nil {
				logger.GetLoggerCtx(cli.Ctx()).Errorf("etcd publisher keepAliveAsync: %s", err.Error())
				break
			}
			...
			case _, ok := <-ch:
				if !ok {
					...
					if err := p.doKeepAlive(); err != nil {
						logger.GetLoggerCtx(cli.Ctx()).Errorf("etcd publisher KeepAlive: %s", err.Error())
					}
					return
				}
			case <-p.pauseChan:
				logger.GetLoggerCtx(cli.Ctx()).Infof("paused etcd renew, key: %s, value: %s", p.key, p.value)
				p.revoke(cli)
				select {
				case <-p.resumeChan:
					if err := p.doKeepAlive(); err != nil {
...
func (p *Publisher) revoke(cli internal.EtcdClient) {
	if _, err := cli.Revoke(cli.Ctx(), p.lease); err != nil {
		logger.GetLoggerCtx(cli.Ctx()).Errorf("etcd publisher revoke: %s", err.Error())
	}
}
...
func WithPubEtcdTLS(certFile, certKeyFile, caFile string, insecureSkipVerify bool) PubOption {
	return func(pub *Publisher) {
		logger.Must(RegisterTLS(pub.endpoints, certFile, certKeyFile, caFile, insecureSkipVerify))
	}
}
