...
import (
	...
	"github.com/meta-atman/metarpc/core/configcenter/subscriber"
...
func (c *configCenter[T]) loadConfig() error {
	...
	if err != nil {
		if c.conf.Log {
			logger.Errorf("ConfigCenter loads changed configuration, error: %v", err)
		}
		return err
	}

	if c.conf.Log {
		logger.Infof("ConfigCenter loads changed configuration, content [%s]", v)
	}
...
func (c *configCenter[T]) genValue(data string) *value[T] {
	...
	case reflect.Struct, reflect.Array, reflect.Slice:
		if err := c.unmarshaler([]byte(data), &v.marshalData); err != nil {
			v.err = err
			if c.conf.Log {
				logger.Errorf("ConfigCenter unmarshal configuration failed, err: %+v, content [%s]",
					err.Error(), data)
			}
		}
	case reflect.String:
	...
	default:
		if c.conf.Log {
			logger.Errorf("ConfigCenter unmarshal configuration missing unmarshaler for type: %s, content [%s]",
				t.Kind(), data)
		}
		v.err = errMissingUnmarshalerType
	}
