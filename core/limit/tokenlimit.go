...
func (lim *TokenLimiter) reserveN(ctx context.Context, now time.Time, n int) bool {
	...
	if errorx.In(err, context.DeadlineExceeded, context.Canceled) {
		logger.Errorf("fail to use rate limiter: %s", err)
		return false
	}
	if err != nil {
		logger.Errorf("fail to use rate limiter: %s, use in-process limiter for rescue", err)
		lim.startMonitor()
		return lim.rescueLimiter.AllowN(now, n)
	}
	...
	code, ok := resp.(int64)
	if !ok {
		logger.Errorf("fail to eval redis script: %v, use in-process limiter for rescue", resp)
		lim.startMonitor()
		return lim.rescueLimiter.AllowN(now, n)
	}
